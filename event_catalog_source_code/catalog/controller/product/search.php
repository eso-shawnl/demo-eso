...
class ControllerProductSearch extends Controller {
    public function index() {
        ...
        if (isset($this->request->post['search'])) {
            ...
            $this->document->setTitle($this->language->get('heading_title') .  ' - ' . $this->language->post('heading_tag') . $this->request->post['tag']);
        } else {
        ...
        if (isset($this->request->post['search'])) {
            $url .= '&search=' . $this->request->post['search'];
        }elseif (isset($this->request->get['search'])) {
            $url .= '&search=' . urlencode(html_entity_decode($this->request->get['search'], ENT_QUOTES, 'UTF-8'));
        }

        if (isset($this->request->post['tag'])) {
            $url .= '&tag=' . $this->request->post['tag'];
        } elseif (isset($this->request->get['tag'])) {
        ...
        if (isset($this->request->post['category_id'])) {
            ...
            $url .= '&description=' . $this->request->get['category_id'];
        }

        if (isset($this->request->post['sub_category'])) {
            $url .= '&sub_category=' . $this->request->post['sub_category'];
        }elseif (isset($this->request->get['sub_category'])) {
            $url .= '&description=' . $this->request->get['sub_category'];
        }

        if (isset($this->request->post['sort'])) {
            $url .= '&sort=' . $this->request->post['sort'];
        }elseif (isset($this->request->get['sort'])) {
            $url .= '&description=' . $this->request->get['sort'];
        }

        if (isset($this->request->post['order'])) {
            $url .= '&order=' . $this->request->post['order'];
        }elseif (isset($this->request->get['order'])) {
            $url .= '&description=' . $this->request->get['order'];
        }

        if (isset($this->request->post['page'])) {
            $url .= '&page=' . $this->request->post['page'];
        }elseif (isset($this->request->get['page'])) {
            $url .= '&description=' . $this->request->get['page'];
        }

        if (isset($this->request->post['limit'])) {
            $url .= '&limit=' . $this->request->post['limit'];
        }elseif (isset($this->request->get['limit'])) {
            $url .= '&description=' . $this->request->get['limit'];
        }
        ...
        if (isset($this->request->post['search']) || isset($this->request->post['tag'])) {
            ...
            foreach ($results as $result) {
                ...
                */
                ...
                $event_description = $this->model_catalog_product->get_event_description($result['event_id']);

                $data['products'][] = array(
                    'product_id'  => $result['event_id'],
                    'thumb'       => $image,
                    'name'        => $result['name'],
                    'description' => utf8_substr(strip_tags(html_entity_decode($event_description[$this->config->get('config_language_id')]['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_event_description_length')) . '..',
                    //'price'       => $price,
                    //'special'     => $special,
                    //'tax'         => $tax,
                    //'rating'      => $result['rating'],
                    'href'        => $this->url->link('product/product', 'event_id=' . $result['event_id'] . $url)
                );
            }
