...
class ModelReportCustomer extends Model {
	...
	public function getTotalCustomersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, created_date FROM `" . DB_PREFIX . "customer` WHERE DATE(created_date) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(created_date)");

		foreach ($query->rows as $result) {
			$customer_data[date('j', strtotime($result['created_date']))] = array(
				'day'   => date('d', strtotime($result['created_date'])),
				'total' => $result['total']
			);
		}
